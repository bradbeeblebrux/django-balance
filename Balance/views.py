...
from Balance.models import Transaction, Owner, Account, Transaction_to_Category, Word_to_Category, Category, Excluded_Category
from Balance.forms import AddFromFileForm, PreviewTransForm
import json
import datetime
import accounts
import random
...
#    template_name = 'Balance/transall.html'
#    def get_queryset(self):
#        return Transaction.objects.all()


def transall(request):
    return render(request, "Balance/transall.html", {"trans": Transaction.objects.all()})
...
def SummaryCategoryView(request, year=None, month=None):
    ...
    return render(request, "Balance/summary.html", {"total": total, "summary": summary})
...
def SummaryCategoryPieChart(request, year=None, month=None):
    ...
    return render(request, "Balance/summary_pie.html", {"summary": summary})


def trans(request, trans_id):
    trans = get_object_or_404(Transaction, pk=trans_id)
    return render_to_response('Balance/trans.html', {'trans': trans})
...
def PreviewTransView(request, key):
    ...
    return render(request, 'Balance/preview_trans.html', {'form': form, 'temp_list': upload_list_obj, 'key': key})
...
def trans_add_from_file(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('balance:preview_trans', key)
    else:
        form = AddFromFileForm()

    return render(request, 'Balance/add_from_file.html', {'form': form, })
